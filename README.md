...
│   ├── search-overlay.html — панель поиска по странице
│   ├── search-in-page.js   — логика поиска, подсветка, навигация
│   ├── breadcrumbs.js     — хлебные крошки (Главная → Раздел → Страница)
...
├── data/
│   └── breadcrumbs.json    — маппинг путей → названий для крошек
├── assets/                 — изображения
...
5. Добавьте страницу в `data/breadcrumbs.json` (в `pages`) — путь и название для хлебных крошек.
...
- **Поиск по странице** — иконка 🔍 в шапке, подсветка совпадений, кнопки «дальше/назад», счётчик. Работает на десктопе и мобильных.
- **Хлебные крошки** — путь «Главная → Раздел → Страница». Клик по разделу ведёт на соответствующую секцию главной.
